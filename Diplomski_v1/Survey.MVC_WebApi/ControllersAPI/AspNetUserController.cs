...
namespace Survey.MVC_WebApi.ControllersAPI
{
  ...
  public class AspNetUserController : ApiController
  {
    ...
    public async Task<HttpResponseMessage> Add(AspNetUser aspNetUser)
    {
      try
      {
        var checkUsername = await AspNetUserRepository.CheckIfUsernameTaken(aspNetUser);
        if (checkUsername == true)
        {
          return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Username taken");
        }
        var checkEmail = await AspNetUserRepository.CheckIfEmailTaken(aspNetUser);
        if (checkEmail == true)
        {
          return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Email taken");
        }
